...
const UpcomingEventsSection = ({ upcomingEvents }: { upcomingEvents: any[] }) => (
    <section className="max-w-[1600px] mx-auto px-6 pb-20">
        <div className="flex items-center justify-between mb-12">
            ...
            <a href="/convening-platforms/events" className="hidden md:inline-flex items-center gap-3 px-6 py-3 border-2 border-[#021d49] text-[#021d49] font-bold rounded-xl hover:bg-[#021d49] hover:text-white transition-all shadow-lg">
                View All Events
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {upcomingEvents.map((event, index) => (
                <div key={index} className="group relative">
                    <div className="relative bg-white rounded-3xl p-8 shadow-xl hover:shadow-2xl transition-all duration-500 border-2 border-stone-200 hover:border-[#021d49] hover:-translate-y-2">
                        <div className="flex items-center justify-between mb-6">
                            ...
                            <span className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-stone-50 to-gray-50 rounded-full border-2 border-stone-200 shadow-sm group-hover:border-[#021d49] transition-colors">
                                <Calendar className="w-4 h-4 text-[#021d49]" />
                                <span className="text-xs text-gray-700 font-bold">{event.date}</span>
                            </span>
                        </div>
                        <h3 className="text-2xl font-bold text-gray-900 mb-3 group-hover:text-[#021d49] transition-colors duration-300">
                            {event.title}
                        </h3>
